...
func Server(ctx *user.GlobalCTX) {
	...
	router.HandleFunc("/hg/foreman/check/{host}/{hgName}", middleware.Chain(hostgroups.GetHGCheckNameHttp, middleware.Token(ctx))).Methods("GET")
	router.HandleFunc("/hg/foreman/check/id/{host}/{id:[0-9]+}", middleware.Chain(hostgroups.GetHGCheckIDHttp, middleware.Token(ctx))).Methods("GET")

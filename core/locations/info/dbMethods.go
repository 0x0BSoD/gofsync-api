...
func Get(host string, ctx *user.GlobalCTX) Dashboard {
	...
	stmt, err := ctx.Config.Database.DB.Prepare("select trend, Success, Failed, RFailed, Total, Last from `goFsync`.`hosts` where host = ?")
	if err != nil {
...
func Update(host string, data Dashboard, ctx *user.GlobalCTX) {
	stmt, err := ctx.Config.Database.DB.Prepare("UPDATE `goFsync`.`hosts` SET `trend` = ?, `Last` = ?, `Success` = ?, `Failed` = ?, `RFailed` = ?, `Total` = ? WHERE (`host` = ?)")
	if err != nil {
		utils.Warning.Println(err)
	}
	defer utils.DeferCloseStmt(stmt)
